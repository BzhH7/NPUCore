...
NPUcore-Ovo 支持对称多处理 (SMP)，采用 Per-CPU 架构设计：

```
┌─────────────────────────────────────────────────────────────┐
│                      全局任务管理                             │
│  ┌──────────────────────────────────────────────────────┐   │
│  │  TASK_MANAGERS: [TaskManager; MAX_CPU_NUM]          │   │
│  │  每个CPU核心维护独立的就绪队列和阻塞队列              │   │
│  └──────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────┘
                          │
         ┌────────────────┼────────────────┐
...
pub struct TaskControlBlock {
    /// 运行在哪个CPU核心上（原子操作）
    pub running_on_cpu: AtomicUsize,  // TASK_NOT_RUNNING 或 CPU ID
}
...
工作窃取策略：
1. 优先从本地队列获取任务
2. 本地队列空时，尝试从其他CPU队列窃取
3. **关键**：只能窃取标记为 `TASK_NOT_RUNNING` 的任务
4. 通过原子CAS操作确保不会窃取正在运行的任务

### 4.5 多级调度框架

NPUcore-Ovo 实现了类似 Linux 的多级调度框架，支持多种调度策略：
...
| 调度策略 | 优先级 | 描述 |
|---------|-------|------|
| SCHED_FIFO | RT 1-99 | 实时FIFO，运行直到阻塞或主动让出 |
| SCHED_RR | RT 1-99 | 实时轮转，有时间片限制 |
| SCHED_NORMAL | Nice -20~19 | CFS公平调度（默认） |
| SCHED_BATCH | Nice -20~19 | 批处理任务（CFS变体） |
| SCHED_IDLE | 最低 | 空闲任务 |

#### CFS (Completely Fair Scheduler)

CFS 使用虚拟运行时 (vruntime) 实现公平调度：
...
**核心算法**：
- 使用红黑树维护就绪任务，按 vruntime 排序
- 总是选择 vruntime 最小的任务运行
- vruntime 增长速度与权重成反比：`Δvruntime = Δruntime × (NICE_0_WEIGHT / weight)`
...
pub fn rust_main(hart_id: usize) -> ! {
    ...
    // 原子操作判断 BSP (Boot Strap Processor)
    let is_bsp = !BOOT_FLAG.swap(true, Ordering::SeqCst);
    
    if is_bsp {
        // BSP 执行完整初始化
        println!("[kernel] BSP (hart {}) initializing...", hart_id);
        ...
        println!("[kernel] BSP initialization complete");
    } else {
        // AP (Application Processor) 等待BSP初始化完成
        println!("[kernel] AP (hart {}) waiting for BSP...", hart_id);
        
        while !AP_CAN_START.load(Ordering::Acquire) {
            core::hint::spin_loop();
        }
        
        println!("[kernel] AP (hart {}) starting...", hart_id);
    }
